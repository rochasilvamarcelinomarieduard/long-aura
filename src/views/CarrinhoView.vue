  ...
  <div class="checkout-page">
      ...
      <div class="card-section">
        ...
        <div class="field-row">
          <div class="field">
            ...
            <input v-model="endereco.telefone" type="text" placeholder="(00) 00000-0000" class="input" maxlength="15" @input="formatarTelefone"/>
          </div>
        </div>
        ...
        <div class="pix-qr">
          <div class="pix-qr-fake">
            <svg viewBox="0 0 100 100" width="100" height="100">
              ...
              <rect x="44" y="10" width="6" height="6" fill="#161823"/>
              <rect x="52" y="10" width="6" height="6" fill="#161823"/>
              <rect x="44" y="18" width="6" height="6" fill="#161823"/>
              <rect x="44" y="44" width="6" height="6" fill="#161823"/>
              <rect x="52" y="44" width="6" height="6" fill="#161823"/>
              <rect x="60" y="44" width="6" height="6" fill="#161823"/>
              <rect x="68" y="44" width="6" height="6" fill="#161823"/>
              <rect x="76" y="44" width="6" height="6" fill="#161823"/>
              <rect x="44" y="52" width="6" height="6" fill="#161823"/>
              <rect x="60" y="52" width="6" height="6" fill="#161823"/>
              <rect x="44" y="60" width="6" height="6" fill="#161823"/>
              <rect x="52" y="60" width="6" height="6" fill="#161823"/>
              <rect x="68" y="60" width="6" height="6" fill="#161823"/>
              <rect x="76" y="60" width="6" height="6" fill="#161823"/>
              <rect x="44" y="68" width="6" height="6" fill="#161823"/>
              <rect x="60" y="68" width="6" height="6" fill="#161823"/>
              <rect x="44" y="76" width="6" height="6" fill="#161823"/>
              <rect x="52" y="76" width="6" height="6" fill="#161823"/>
              <rect x="60" y="76" width="6" height="6" fill="#161823"/>
              <rect x="76" y="76" width="6" height="6" fill="#161823"/>
            </svg>
          </div>
    ...
    <div v-if="step === 4" class="step-content confirmacao">
      ...
      <div class="card-section">
        ...
        <div class="resumo-row"><span>Itens</span><span>{{ itensSelecionados.length }}</span></div>
        <div class="resumo-row">
          <span>Entrega</span>
    ...
    <div class="bottom-bar" v-if="step < 4 && !carrinho.isEmpty()">
      ...
      <button class="checkout-btn" :disabled="!podeAvancar" @click="avancar">
        {{ labelBotao }}
      </button>
    </div>
...
const podeAvancar = computed(() => {
  ...
  if (step.value === 3) {
    if (pagamentoSelecionado.value === 'cartao')
      return cartao.value.numero.length >= 19 && cartao.value.nome && cartao.value.validade.length === 5 && cartao.value.cvv.length >= 3
    return true
  }
  return false
})
...
function avancar() {
  ...
  if (step.value === 3) {
    // Finaliza pedido
    numeroPedido.value = '#' + Math.random().toString(36).slice(2, 8).toUpperCase()
    carrinho.limpar()
    selecionados.value = []
    step.value = 4
    return
...
/* ─── Progress bar ─── */
.progress-bar {
  height: 3px; background: var(--gray-2); position: relative;
}
.progress-fill {
  height: 100%; background: #FE2C55;
...
/* ─── Cards ─── */
.card-section {
  background: white; margin: 8px 0; padding: 16px 14px;
}
.section-title {
...
.tts-check {
  ...
  width: 20px; height: 20px; border: 1.8px solid #ccc; border-radius: 50%;
...
.loja-badge {
  ...
  background: #e6faf0; padding: 2px 7px; border-radius: 20px;
}

/* ─── Item ─── */
.item-row {
  display: flex; align-items: flex-start; gap: 10px;
  padding: 14px; border-bottom: 1px solid var(--gray);
}
...
.item-qty-badge {
  ...
  background: #FE2C55; color: white; font-size: 10px; font-weight: 700;
...
.qty-btn { width: 28px; height: 28px; background: var(--gray); border: none; font-size: 17px; font-weight: 300; cursor: pointer; color: var(--text); display: flex; align-items: center; justify-content: center; transition: background .1s; line-height: 1; }
...
.cep-btn {
  flex-shrink: 0; width: 40px; height: 40px; background: var(--gray);
  border: 1px solid var(--border); border-radius: 8px;
  cursor: pointer; display: flex; align-items: center; justify-content: center;
...
.endereco-resumo {
  display: flex; align-items: flex-start; gap: 10px;
  background: var(--gray); border-radius: 8px; padding: 12px;
}
...
.entrega-option {
  ...
  padding: 14px 12px; margin-bottom: 10px; cursor: pointer;
  transition: border-color .15s, background .15s;
}
.entrega-option:hover { border-color: #FE2C55; background: #fff5f7; }
...
.pagamento-option:hover { border-color: #FE2C55; background: #fff5f7; }
...
.copiar-pix-btn {
  ...
  background: var(--gray); border: 1px solid var(--border); border-radius: 20px;
...
.confirmacao { }
.confetti-area {
  background: white; padding: 40px 20px 32px;
...
.empty-state {
  ...
  min-height: 60vh; padding: 40px 24px; text-align: center; gap: 12px;
}
...
.empty-cta {
  margin-top: 8px; background: #FE2C55; color: white; border: none;
  border-radius: 24px; padding: 12px 32px; font-size: 15px; font-weight: 600;
  cursor: pointer; transition: opacity .15s;
}
...
.checkout-btn {
  flex-shrink: 0; background: #FE2C55; color: white; border: none;
  border-radius: 24px; padding: 0 22px; height: 44px;
